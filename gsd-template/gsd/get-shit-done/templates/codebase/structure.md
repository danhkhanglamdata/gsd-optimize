...
- Usage: Reference from command with `@C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/workflows/{name}.md`
...
**get-shit-done/**
- Purpose: Resources installed to C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/
- Source: Copied by bin/install.js during installation
- Committed: Yes (source of truth)

**commands/**
- Purpose: Slash commands installed to C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/commands/
- Source: Copied by bin/install.js during installation
- Committed: Yes (source of truth)
...
- Cross-reference to related templates (see "Related Templates" section)
- Code implementation details (defer to code reading)
- Every single file (focus on directories and key files)

## Related Templates

This template is part of `.planning/codebase/` set. AI agents use these for context when planning.

| Template | Purpose | Agent Use Case |
|----------|---------|----------------|
| `STRUCTURE.md` | Physical file organization | "Where do I create X file?" |
| `ARCHITECTURE.md` | Conceptual design & module boundaries | "Which module owns X?" |
| `CONVENTIONS.md` | Coding standards & patterns | "How do I name/write X?" |
| `STACK.md` | Technology choices & versions | "What tools/versions to use?" |
| `TESTING.md` | Test strategy & patterns | "How do I test X?" |
| `INTEGRATIONS.md` | External services & APIs | "How do we connect to X?" |
| `CONCERNS.md` | Risks, constraints & decisions | "What should we watch for?" |

**Agent context flow:**
1. gsd-planner reads STACK.md → knows technologies
2. gsd-planner reads ARCHITECTURE.md → knows module design
3. gsd-planner reads STRUCTURE.md → knows file locations
4. gsd-planner reads CONVENTIONS.md → knows naming rules
5. gsd-executor reads CONCERNS.md → avoids pitfalls
...
- gsd-planner uses this to determine file paths in PLAN.md
- gsd-executor uses this to place new files correctly
- Include "Where to Add New Code" section - critical for agents
- Use exact paths (relative to project root) for agent parsing
