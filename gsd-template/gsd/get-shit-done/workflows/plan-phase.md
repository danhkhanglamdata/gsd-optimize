...
```bash
PHASE_INFO=$(node "C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node "C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_DESC=$(node "C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" | jq -r '.section')
```
...
**If found:**
1. Read template: `C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/templates/VALIDATION.md`
...
```bash
UI_PHASE_CFG=$(node "C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(node "C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node "C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" 2>/dev/null)
...
2. Disable Nyquist with the exact command:
   `node "C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/bin/gsd-tools.cjs" config-set workflow.nyquist_validation false`
...
- .planning/codebase/STRUCTURE.md (File placement rules — MANDATORY for all file path decisions)
- .planning/codebase/CONVENTIONS.md (Naming, CSS, stack execution rules — MANDATORY for all task <action> specs)
</files_to_read>

**Phase requirement IDs (every ID MUST appear in a plan's `requirements` field):** {phase_req_ids}

**Project instructions:** Read ./CLAUDE.md if exists — follow project-specific guidelines
**Codebase rules:** STRUCTURE.md and CONVENTIONS.md above are ground truth for file paths and stack decisions. Every task `<files>` path must comply with STRUCTURE.md. Every task `<action>` for a .tsx file must specify SC/CC and Supabase client per CONVENTIONS.md Stack Execution Rules.
</planning_context>
...
- .planning/codebase/STRUCTURE.md (File placement rules — for Dimension 9 Blueprint Compliance)
- .planning/codebase/CONVENTIONS.md (Stack execution rules — for Dimension 9 Blueprint Compliance)
</files_to_read>
