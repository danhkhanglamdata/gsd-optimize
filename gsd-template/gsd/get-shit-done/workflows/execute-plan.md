...
@C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/references/git-integration.md
</required_reading>
...
```bash
node "C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/bin/gsd-tools.cjs" phases list --type summaries --raw
...
See `C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/references/tdd.md` for structure.
</tdd_plan_execution>
...
**3. TypeScript gate (before commit):**
```bash
[ -f tsconfig.json ] && npx tsc --noEmit 2>&1 | head -20 || echo "No tsconfig — skip"
```
If TS errors found → fix before committing. Do NOT commit with TypeScript errors.
...
**5. Format:** `{type}({phase}-{plan}): {description}` with bullet points for key changes.
...
**7. Check for untracked generated files:**
```bash
git status --short | grep '^??'
```
If new untracked files appeared after running scripts or tools, classify each:

| Category | Examples | Action |
|---|---|---|
| Source file | `.ts`, `.tsx`, `.css`, `.json`, `.md` | **Commit it** |
| Config | `next.config.ts`, `tailwind.config.ts`, `components.json` | **Commit it** |
| Public asset | files inside `public/` folder | **Commit it** |
| Build output | `.next/`, `dist/`, `*.tsbuildinfo` | **Add to .gitignore** |
| Runtime output | `.env`, cache files, compiled output | **Add to .gitignore** |
| **Test artifact** | `*.png`, `*.jpg`, `*.jpeg` at project root, `console.log`, `network.log`, `*.log` at root, `test-image.*`, `test-file.*` | **DELETE — never commit, never gitignore** |
| **Debug artifact** | `*-screenshot.png`, Playwright output at root, MCP tool outputs | **DELETE — never commit, never gitignore** |

```bash
# Delete test/debug artifacts if found at project root
for f in *.png *.jpg *.jpeg *.log; do
  [ -f "$f" ] && echo "Deleting artifact: $f" && rm "$f"
done
```

Do NOT leave any untracked files without a decision.
...
See C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/templates/summary.md`.
...
node "C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/bin/gsd-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
node "C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/bin/gsd-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
