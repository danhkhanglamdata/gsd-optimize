...
**Creative Enhancements (if generated in analyze_phase):**
If you generated creative enhancement ideas using ux-brainstormer, present them as a separate multi-select option:
```
## 💡 Creative Delight Moments

Want to add special UI moments to [feature]? Based on latest trends:

☐ [Enhancement 1] — [description]
☐ [Enhancement 2] — [description]
☐ [Enhancement 3] — [description]

These are optional — adds "wow" factor to the experience.
```
...
```
## Phase ${PHASE}: [Name] — Context Ready

Your decisions have been captured. What's next?
```
...
- options:
  - "UI design contract — run /gsd:ui-phase ${PHASE} first (recommended for frontend phases)"
  - "Skip to planning — run /gsd:plan-phase ${PHASE} directly"
  - "View context — review CONTEXT.md before deciding"
  - "Edit context — modify decisions before continuing"

**Handle user choice:**

- **"UI design contract"**: Proceed to git_commit, then invoke ui-phase:
  ```
  Skill(skill="gsd:ui-phase", args="${PHASE}")
  ```

- **"Skip to planning"**: Proceed to git_commit, then invoke plan-phase:
  ```
  Skill(skill="gsd:plan-phase", args="${PHASE}")
  ```

- **"View context"**: Display CONTEXT.md, then re-prompt the same AskUserQuestion.

- **"Edit context"**: Open CONTEXT.md in editor, wait for user to save, then re-prompt the same AskUserQuestion.
...
2. **Sync chain flag with intent** — if user invoked manually (no `--auto`), clear the ephemeral chain flag from any previous interrupted `--auto` chain. This does NOT touch `workflow.auto_advance` (the user's persistent settings preference):
   ...
   if [[ ! "$ARGUMENTS" =~ --auto ]]; then
     node "C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false 2>/dev/null
   fi
   ```
...
```bash
node "C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active true
```

**If `--auto` flag present OR `AUTO_CHAIN` is true OR `AUTO_CFG` is true:**

Check if ui_phase is enabled (default: true for auto-advance):
```bash
UI_AUTO=$(node "C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase_auto 2>/dev/null || echo "true")
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GSD ► AUTO-ADVANCING TO UI PHASE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Context captured. Generating UI design contract first...
```

Launch ui-phase using the Skill tool:
```
Skill(skill="gsd:ui-phase", args="${PHASE} --auto")
```
...
Launch plan-phase using the Skill tool:
```
Skill(skill="gsd:plan-phase", args="${PHASE} --auto")
```

This keeps the auto-advance chain flat — discuss, ui-phase, plan, and execute all run at the same nesting level rather than spawning increasingly deep Task agents.

**Handle ui-phase return (when UI_AUTO is true):**
After ui-phase completes successfully:
- Display: "UI design contract generated. Proceeding to planning..."
- Launch plan-phase:
  ```
  Skill(skill="gsd:plan-phase", args="${PHASE} --auto --no-transition")
  ```
- The `--no-transition` flag prevents ui-phase from triggering another auto-advance cycle
...
- User prompted for next step (ui-phase vs plan-phase vs view/edit context)
- Auto-advance routes to ui-phase first (if enabled), then to plan-phase
