...
**Project skills (MUST LOAD):**

1. **ux-brainstormer** — Creative UI/UX brainstorming and gamification research
   ```
   Read: .claude/skills/ux-brainstormer/SKILL.md
   ```
   Use this to generate creative enhancements and research gamification patterns

2. **style-adapter** — Analyze inspiration sites and convert designs
   ```
   Read: .claude/skills/style-adapter/SKILL.md
   ```
   Use this when user provides inspiration URLs or needs design conversion

3. **beautiful-ui-generator** — Component patterns and animations
   ```
   Read: .claude/skills/beautiful-ui-generator/SKILL.md
   ```
   Use this for component suggestions and animation specs

**Workflow:**
1. Load all 3 skills at start
2. Search for latest UI/UX trends using WebSearch
3. Generate creative enhancement suggestions
4. Include in UI-SPEC.md under "## Creative Elements"
</project_context>
...
## Creative Enhancement with ux-brainstormer

Before asking design contract questions, enhance the phase with creative UI/UX suggestions:

**Step 1: Load ux-brainstormer skill**
```
Read .claude/skills/ux-brainstormer/SKILL.md
```
...
**Step 3: Generate creative suggestions**
Based on the phase feature, propose 2-3 creative enhancements:

For example:
- Quiz phase → "confetti burst on correct answer", "countdown with pulse animation", "leaderboard rank change animation"
- Media wall → "photo slide-in with stagger", "reaction emoji burst", "pull-to-refresh with sparkle"
- Gamification → "wheel spin physics", "energy bar fill with glow", "achievement unlock animation"

**Step 4: Present as options**
Ask user which creative enhancements they want:
```
...
3. **[Enhancement name]** — [description + visual effect]

Which would you like to include? (Select all that apply)
```

Include these in the UI-SPEC.md under "## Creative Elements" section.
