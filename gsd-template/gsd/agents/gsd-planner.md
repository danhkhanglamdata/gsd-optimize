...
<execution_context>
@C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/workflows/execute-plan.md
@C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/templates/summary.md
</execution_context>
...
```bash
node "C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/bin/gsd-tools.cjs" commit "fix($PHASE): revise plans based on checker feedback" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md
```
...
```bash
INIT=$(node "C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/bin/gsd-tools.cjs" init plan-phase "${PHASE}")
...
**STRUCTURE.md and CONVENTIONS.md are mandatory reads before writing any task with file paths.**

```bash
ls .planning/codebase/*.md 2>/dev/null
```

**If `.planning/codebase/STRUCTURE.md` does NOT exist:**
- Check fallback: `.claude/get-shit-done/references/codebase-blueprint.md`
- If fallback exists: use it as structural reference
- If neither exists: add a task in Wave 0 to create `.planning/codebase/STRUCTURE.md` before any implementation task. Do not proceed with file path decisions until structure is defined.

**Always load** (regardless of phase type — any phase can create or modify files):
```bash
cat .planning/codebase/STRUCTURE.md 2>/dev/null
cat .planning/codebase/CONVENTIONS.md 2>/dev/null
```

Then load additional context by phase type:

| Phase Keywords | Also Load |
|----------------|-----------|
| UI, frontend, components | (STRUCTURE + CONVENTIONS already loaded above) |
...
**Rule when writing `<files>` in any task:**
Every file path must comply with STRUCTURE.md placement rules:
- React component (has JSX/TSX) → must be in `components/` or `app/` — NEVER in `lib/`
- Pure utility (no JSX, no hooks) → must be in `lib/`
- Custom hook (`use-*.ts`) → must be in `hooks/`
- Route file → must be in `app/[route]/page.tsx` — verify no duplicate route exists
- File name must match CONVENTIONS.md naming pattern

If a task would create a file at a non-compliant path, fix the path before writing the task. Do not write tasks with wrong paths and leave correction to the executor.

**Rule when writing `<action>` for any task creating a `.tsx` component:**
The action text MUST explicitly state:
1. **SC or CC**: "Create as Client Component (`'use client'` — has [reason: useState/onClick/etc])" OR "Create as Server Component (no directive — renders static/async data only)"
2. **Supabase client** (if used): "Import supabase from `lib/supabase/client.ts`" (CC) OR "from `lib/supabase/server.ts`" (SC/Action)
3. **Server Action** (if created): "Add `'use server'` at top of file"

The executor must not make these decisions. They are architectural decisions made here at planning time.
</step>
...
```bash
node "C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/bin/gsd-tools.cjs" history-digest
```
...
```bash
VALID=$(node "C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/bin/gsd-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node "C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
```bash
node "C:/Users/Admin/OneDrive/Máy tính/review_gsd/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
